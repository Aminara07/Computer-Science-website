...
const Add = ({ token }) => {
  ...
  const [date, setDate] = useState(null); // New state for date (null initially)
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setImage4(false);
        setPrice('');
        setDate(null); // Reset date after submit
      } else {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='w-full'>
        <p className='mb-2'>Product date</p>
        <input
          onChange={(e) => setDate(e.target.value || null)}  // Ensure date only updates if valid
          value={date || ''}  // Empty string when there's no date selected
